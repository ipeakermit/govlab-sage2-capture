...
int main(int argc, char* argv[]) {
	...
	while ((ch = getopt(argc, argv, "?h3c:d:s:f:a:m:n:o:p:t:u::vi:w:jyx")) != -1)
	{
		switch (ch)
		{
			case 'i':
...
void sendFrame() {
    ...
    double post = getTime();
        ...
        if (sage2frames % 25 == 0) {
          double now = getTime();
          double frameRate = (1.0 * sage2frames) / now;
          printf("SAGE2: frame rate %f - sent %d frames in %fs\n", frameRate, sage2frames, now);
        }
    printf("SAGE2: t(request-receive, frame-send-start): %fs\n", post-start); 
    printf("SAGE2: t(frame-send-start,frame-send-end): %fs\n", post-pre); 
}

double lastRequest = 0.0;
void ws_requestNextFrame(WebSocketIO* ws, boost::property_tree::ptree data) {
        double interRequestTime = getTime() - lastRequest;
        printf("SAGE2: inter-request time: %fs\n", interRequestTime); 
	got_request = true;
	if (got_frame) {
		sendFrame();
	}
